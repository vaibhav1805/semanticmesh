...
// LoadGraphmdIgnore reads a .graphmdignore file from the given directory.
...
func LoadGraphmdIgnore(dir string) (dirs []string, files []string, err error) {
	...
	if err != nil {
		...
		return nil, nil, fmt.Errorf("knowledge.LoadGraphmdIgnore: open %q: %w", path, err)
	}
	defer f.Close()
	...
	if err := scanner.Err(); err != nil {
		return nil, nil, fmt.Errorf("knowledge.LoadGraphmdIgnore: read %q: %w", path, err)
	}

	return dirs, files, nil
}

// GenerateGraphmdIgnore creates a .graphmdignore file with default patterns
// in the given directory. If the file already exists, it is not overwritten.
func GenerateGraphmdIgnore(dir string) error {
	path := filepath.Join(dir, ".graphmdignore")
	...
	b.WriteString("# .graphmdignore — patterns for files and directories to exclude from scanning.\n")
	b.WriteString("#\n")
